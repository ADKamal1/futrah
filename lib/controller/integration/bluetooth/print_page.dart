...
class PrintPage extends StatefulWidget {
  final int id;
  const PrintPage({super.key, required this.id});
...
class _PrintPageState extends State<PrintPage> {
  ...
  void initState() {
    super.initState();
    BillsCubit.get(context).getSingleBill(id: widget.id);
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      ElevatedButton(
                        onPressed: _isLoading ? null : onScanPressed,
                        child: MyText(
                            title: 'refresh'.tr(context), color: Colors.white),
                      )
                    ],
                  ),
                ),
  ...
  Future<void> _onPrintReceipt() async {
    final allM = BillsCubit.get(context).singleBillModel;
    final model = BillsCubit.get(context)
        .singleBillModel!
        .data!
        .where((element) => element.id == widget.id)
        .first;
    final finalPrice = model.finalPrice!;
    final pbTax = model.priceBeforeTax;
    final taxA = model.taxAmount;
    
print("fadfdsffsdfdsfdfdfsdf+sdfsd+fdfdsdf"+ allM!.data!.first.finalPrice.toString());
    ...
    for (var i = 0; i < model.products!.length; i++) {
      r.addLeftRightText('${title(model, i)}', 'اسم المنتج');
      r.addLeftRightText('${quantity(model, i)}', 'الكمية');
      r.addLeftRightText('${price(model, i)}', 'السعر');
    }
    r.addSpacer(useDashed: true);
    r.addText('${'$finalPrice SAR'}مبلغ الفاتوره الاجمالي ',
        alignment: ReceiptAlignment.left);
    r.addSpacer(useDashed: true);
    r.addLeftRightText('$pbTax$sarAr', tpOutTaxAr);
    r.addLeftRightText('$pbTax SAR', tpOutTaxEn);
    r.addLeftRightText('$pbTax$sarAr', totalVATAr);
    r.addLeftRightText('$pbTax SAR', totalVATEn);
    r.addLeftRightText('$taxA$sarAr', totalVATP);
    r.addLeftRightText('$taxA SAR', 'Total VAT (15%)');
    r.addSpacer(useDashed: true);
    ...
    await _bluePrintPos.printQR(allM.qrBase64!, size: 200).then((value) {
      //print change
      return pushNamed(context, route: Routes.category);
    });
    r.addSpacer(useDashed: true);
  }

  String? title(Data model, int i) => model.products![i].title;
  int? price(Data model, int i) => model.products![i].price;
  int? quantity(Data model, int i) => model.products![i].pivot!.quantity;
  String get getDeviceAddress => _bluePrintPos.selectedDevice!.address;
}

String get simpleTaxEn => 'Simple Tax Invoice';
String get simpleTaxAr => 'فاتورة ضريبة مبسطة';
String get totalVATEn => 'Total Amount after discount (VAT Exclusive)';
String get totalVATAr =>
    'المبلغ الإجمالي بعد الخصم(غير شامل ضريبة القيمة المضافة)';
String get tpOutTaxAr => 'اجمالي المبلغ (غير شامل ضريبه القيمه المضافه)';
String get tpOutTaxEn => 'Total Amount (VAT Exclusive)';
String get sarAr => 'ريال سعودي ';
String get anon => 'anonymous';
String get totalVATP => 'مجموع ضريبة القيمة المضافة (15%)';
